...
class MidjourneyGenerator(AIModelGenerator):
    ...
    async def handle_upscales(self, session, message_id, upscale_buttons, task_id):
        ...
        if job_id:
            await self.start_polling_for_upscales(task_id, job_id)
        return results
    ...
    # Add to MidjourneyGenerator class
    async def start_polling_for_upscales(self, task_id, job_id):
        """Start a background task to poll for upscaled images"""
        app.logger.error(f"Starting polling for upscales: task_id={task_id}, job_id={job_id}")
        asyncio.create_task(self.poll_for_upscales(task_id, job_id))

    async def poll_for_upscales(self, task_id, job_id):
        """Poll Discord for upscaled images and update Redis when found"""
        max_attempts = 30
        redis = await self.get_async_redis()

        try:
            app.logger.error(f"Begin polling for upscales: task_id={task_id}, job_id={job_id}")

            for attempt in range(max_attempts):
                app.logger.error(f"Polling attempt {attempt + 1}/{max_attempts}")

                # Create a new session for polling
                async with aiohttp.ClientSession() as session:
                    # Get the latest messages from Discord
                    async with session.get(
                            f"https://discord.com/api/v9/channels/{self.channel_id}/messages?limit=20",
                            headers={"Authorization": self.auth_token}
                    ) as response:
                        if response.status == 200:
                            messages = await response.json()

                            # Check each message for upscaled images
                            for message in messages:
                                author_id = message.get('author', {}).get('id')
                                content = message.get('content', '')
                                attachments = message.get('attachments', [])

                                # Look for messages from Midjourney bot that contain upscaled images
                                if (author_id == self.MIDJOURNEY_BOT_ID and
                                        job_id in content and
                                        "Upscaled" in content and
                                        attachments):

                                    # Extract variation number from the content
                                    # Format: "Image #1 Upscaled (xxx%)"
                                    variation = None
                                    for part in content.split():
                                        if part.startswith('#'):
                                            try:
                                                variation = int(part[1:])
                                                break
                                            except ValueError:
                                                pass
                                    ...
                                    await self.store_upscaled_image(task_id, image_url, variation, redis)
                                    app.logger.error(f"Found upscaled image {variation} at {image_url}")

                # Check if all images are found
                group_key = f"midjourney_group:{task_id}"
                group_data = await redis.get(group_key)

                if group_data:
                    group_info = json.loads(group_data)
                    if len(group_info.get('images', [])) >= 4:
                        app.logger.error(f"All 4 upscaled images found for task {task_id}")
            ...
            app.logger.error(f"Error in poll_for_upscales: {str(e)}")
        finally:
            await redis.close()

    async def store_upscaled_image(self, task_id, image_url, variation, redis):
        """Store an upscaled image in Redis"""
        try:
            # Get the group data
            group_key = f"midjourney_group:{task_id}"
            group_data = await redis.get(group_key)
            ...
            if group_data:
                ...
                for img in group_info.get('images', []):
                    if img.get('variation_number') == variation:
                ...
                # Get or initialize images array
                ...
                app.logger.error(f"Stored upscaled image {variation} for task {task_id}")
                return True
